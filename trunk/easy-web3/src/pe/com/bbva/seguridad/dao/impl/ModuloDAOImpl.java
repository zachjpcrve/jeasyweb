...
public class ModuloDAOImpl extends GenericDAOImpl<Modulo> implements ModuloDAO {
	...
	private static String LISTAR_MODULOS_POR_USUARIO = "SELECT A.* FROM (SELECT MO.* FROM EWTEST.TEWTEST_USUARIO US "
			...
			+ "LEFT JOIN EWTEST.TEWTEST_MODULO MO "
	...
	public List<Modulo> findModulos(Modulo modulo) throws Exception,
		...
		List<Modulo> listaModulos = super.executeQuery(Modulo.class, where,
				orders);
		return listaModulos;
	}
